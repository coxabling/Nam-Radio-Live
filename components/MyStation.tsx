...
export const BADGES: Badge[] = [
  { id: 'superfan', name: 'Superfan', description: 'Listen for over 10 hours in a month.', icon: SuperfanIcon, isEarned: (stats) => stats.monthlyListeningTime > 36000 },
  { id: 'night_owl', name: 'Night Owl', description: 'Tune in after midnight.', icon: NightOwlIcon, isEarned: (stats) => stats.hasListenedPostMidnight },
  { id: 'tastemaker', name: 'Tastemaker', description: 'Request 5 or more songs.', icon: TastemakerIcon, isEarned: (stats, songRequests) => songRequests.length >= 5 },
  { id: 'critic', name: 'The Critic', description: 'Rate 20 or more songs.', icon: CriticIcon, isEarned: (stats) => (stats.likedSongs.length + stats.dislikedSongs.length) >= 20 },
  { id: 'chatterbox', name: 'Chatterbox', description: 'Send 20 messages in the live chat.', icon: ChatterboxIcon, isEarned: (stats) => (stats.chatMessagesSent || 0) >= 20 },
  { id: 'engaged', name: 'Engaged Listener', description: 'Vote in 5 polls or takeovers.', icon: EngagedIcon, isEarned: (stats) => (stats.votesCast || 0) >= 5 },
  { id: 'early_bird', name: 'Early Bird', description: 'Listen to "Sunrise Beats" for over an hour.', icon: EarlyBirdIcon, isEarned: (stats) => (stats.showListeningTime['Sunrise Beats'] || 0) > 3600 }
];
...
const Marquee: React.FC<{ children: React.ReactNode }> = ({ children }) => (
    <div className="relative flex overflow-x-hidden bg-slate-800/50 border-y border-amber-500/30 py-3 group">
        <div className="flex animate-marquee whitespace-nowrap text-amber-300 font-semibold group-hover:[animation-play-state:paused]">
            <span className="mx-8">{children}</span><span className="mx-8">{children}</span>
        </div>
...
const MyStation: React.FC<MyStationProps> = (props) => {
  const { favoriteShows, favoriteDjs, allShows, onToggleFavorite, onToggleFavoriteDj, currentShowName, currentUser, onUpdateUserProfile, songRequests, listeningStats, dailyShowsListened, liveNowPlaying } = props;
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <main className="mt-12 space-y-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          <div className="lg:col-span-1"><UserProfileCard currentUser={currentUser} onUpdateUserProfile={onUpdateUserProfile} listeningStats={listeningStats}/></div>
          <div className="lg:col-span-2"><ListenerStatsCard listeningStats={listeningStats} onGetRewind={handleGetRewind}/></div>
        </div>
        <ListeningDNA listeningStats={listeningStats} songRequests={songRequests} />
        <BadgesGrid listeningStats={listeningStats} songRequests={songRequests}/>
        <RecommendationsCard favoriteShows={favoriteShows} allShows={allShows} songRequests={songRequests} listeningStats={listeningStats} />
        <FavoritesGrid favoriteShows={favoriteShows} favoriteDjs={favoriteDjs} onToggleFavorite={onToggleFavorite} onToggleFavoriteDj={onToggleFavoriteDj} currentShowName={currentShowName}/>
        <RecentRequestsList songRequests={songRequests} />
      </main>

      {isRewindModalOpen && <DailyRewindModal isLoading={isRewindLoading} rewindContent={rewindContent} error={rewindError} onClose={() => setIsRewindModalOpen(false)}/>}
    </div>
  );
};

// Sub-components for better organization and performance
const UserProfileCard = memo(({ currentUser, onUpdateUserProfile, listeningStats }: { currentUser: User, onUpdateUserProfile: MyStationProps['onUpdateUserProfile'], listeningStats: ListeningStats }) => {
    const [isEditing, setIsEditing] = useState(false);
    const [saveSuccess, setSaveSuccess] = useState(false);
    const [profileData, setProfileData] = useState({ username: currentUser.username, avatarUrl: currentUser.avatarUrl || '', bio: currentUser.bio || '' });
    const [avatarError, setAvatarError] = useState(false);
    const currentLevel = useMemo(() => [...LISTENER_LEVELS].reverse().find(l => listeningStats.points >= l.minPoints) || LISTENER_LEVELS[0], [listeningStats.points]);

    useEffect(() => {
        if (!isEditing) setProfileData({ username: currentUser.username, avatarUrl: currentUser.avatarUrl || '', bio: currentUser.bio || '' });
    ...
    return (
        <section className="bg-slate-900/50 backdrop-blur-xl rounded-2xl p-6 shadow-lg border border-slate-700/50 h-full">
            <div className="flex justify-between items-start">
                ...
                {isEditing ? (
                    <div className="flex gap-2">
                        ...
                        <button onClick={() => setIsEditing(false)} className="p-2 text-red-400 hover:bg-red-500/10 rounded-full" aria-label="Cancel editing"><CancelIcon /></button>
                    </div>
                ) : (
            ...
            <div className="mt-6 text-center">
                <div className="relative w-24 h-24 mx-auto mb-4">
                    ...
                    <div className={`absolute bottom-1 right-1 px-2 py-0.5 rounded-full text-white text-xs font-bold ${currentLevel.color.replace('ring-', 'bg-')}`}>{currentLevel.name}</div>
                </div>
                {isEditing ? (
...
const ListenerStatsCard = memo(({ listeningStats, onGetRewind }: { listeningStats: ListeningStats, onGetRewind: () => void }) => {
    const currentLevel = useMemo(() => [...LISTENER_LEVELS].reverse().find(l => listeningStats.points >= l.minPoints) || LISTENER_LEVELS[0], [listeningStats.points]);
    const nextLevel = useMemo(() => LISTENER_LEVELS.find(l => l.minPoints > listeningStats.points), [listeningStats.points]);
    ...
    return (
        <section className="bg-slate-900/50 backdrop-blur-xl rounded-2xl p-6 shadow-lg border border-slate-700/50 h-full flex flex-col justify-between">
            <div>
                ...
                <div className="mb-4">
                    ...
                    <div className="w-full bg-slate-700 rounded-full h-2.5"><div className={`bg-amber-500 h-2.5 rounded-full`} style={{ width: `${progressPercentage}%` }}></div></div>
            ...
            <button onClick={onGetRewind} className="w-full text-center py-3 px-4 bg-amber-500/20 text-amber-300 font-semibold rounded-lg hover:bg-amber-500/30 transition-colors">Get Your Daily Rewind</button>
        </section>
    );
});

const BadgesGrid = memo(({ listeningStats, songRequests }: { listeningStats: ListeningStats, songRequests: SongRequestRecord[] }) => (
    <section>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-4">
            {BADGES.map(badge => {
                ...
                return (
                    <div key={badge.id} className={`p-4 rounded-lg text-center transition-all duration-300 ${isEarned ? 'bg-amber-500/10' : 'bg-slate-800/50'}`} title={`${badge.name}: ${badge.description}`}>
                        <div className={`mx-auto mb-2 ${isEarned ? 'text-amber-400' : 'text-slate-600'}`}><badge.icon /></div>
...
const RecommendationsCard = memo(({ favoriteShows, allShows, songRequests, listeningStats }: Pick<MyStationProps, 'favoriteShows' | 'allShows' | 'songRequests' | 'listeningStats'>) => {
    ...
    const handleGetRecommendations = async () => {
        ...
        try {
            ...
            const result = await getShowRecommendations(favShowNames, allShowNames, songRequests, listeningStats.likedSongs.map(s => s.id), listeningStats.dislikedSongs.map(s => s.id));
            setRecommendations(result);
        } catch (err) { setError('Failed to get recommendations. Please try again.'); } finally { setIsLoading(false); }
    };
                ...
                {recommendations ? (
                    ...
                    !isLoading && (
                        <div className="text-center">
                            ...
                            <button onClick={handleGetRecommendations} className="px-4 py-2 bg-amber-500 text-white font-semibold rounded-lg shadow-md hover:bg-amber-600 transition-all duration-200">Ask DJ Alex</button>
                        </div>
                    )
                )}
            </div>
...
const FavoritesGrid = memo(({ favoriteShows, favoriteDjs, onToggleFavorite, onToggleFavoriteDj, currentShowName }: Pick<MyStationProps, 'favoriteShows' | 'favoriteDjs' | 'onToggleFavorite' | 'onToggleFavoriteDj' | 'currentShowName'>) => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
        <section className="bg-slate-900/50 backdrop-blur-xl rounded-2xl p-6 shadow-lg border border-slate-700/50 flex flex-col h-full">
            <h2 className="text-2xl font-bold mb-6 tracking-wide text-white">Favorite Shows</h2>
            {favoriteShows.length > 0 ? (
                <ul className="space-y-3 flex-grow">
                    {favoriteShows.map(show => <li key={show.id} className={`p-3 rounded-lg flex items-center justify-between transition-all duration-300 ${show.name === currentShowName ? 'bg-amber-500/10 ring-2 ring-amber-400' : 'bg-slate-800/50'}`}><div className="flex items-center gap-3 overflow-hidden"><button onClick={() => onToggleFavorite(show.id)} className="p-1 text-amber-400 flex-shrink-0" aria-label="Remove from favorites"><StarIcon filled={true} /></button><div className="overflow-hidden"><h4 className="font-semibold text-white truncate">{show.name}</h4><p className="text-xs text-slate-400">{new Date(show.start).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}</p></div></div>{show.name === currentShowName && <span className="px-2 py-1 text-xs font-bold uppercase tracking-wider text-black bg-amber-300 rounded-full flex-shrink-0">On Air</span>}</li>)}
                </ul>
            ) : (
                <div className="flex-grow flex flex-col items-center justify-center text-center p-8 bg-slate-800/50 rounded-lg border-2 border-dashed border-slate-700"><div className="text-amber-400 mb-3"><MusicIcon /></div><h4 className="font-semibold text-white">No Favorite Shows Yet</h4><p className="text-sm text-slate-400 mt-1">Star a show from the schedule to see it here!</p></div>
            )}
        </section>
        <section className="bg-slate-900/50 backdrop-blur-xl rounded-2xl p-6 shadow-lg border border-slate-700/50 flex flex-col h-full">
            ...
            {favoriteDjs.length > 0 ? (
                <ul className="space-y-3 flex-grow">
                    {favoriteDjs.map(dj => <li key={dj.id} className={`p-3 rounded-lg flex items-center gap-3 ${dj.show === currentShowName ? 'bg-amber-500/10' : 'bg-slate-800/50'}`}><img src={dj.imageUrl} alt={dj.name} className="w-10 h-10 rounded-full object-cover"/><div><h4 className="font-semibold text-white">{dj.name}</h4><p className="text-xs text-amber-400">{dj.show}</p></div><button onClick={() => onToggleFavoriteDj(dj.id)} className="p-1 text-amber-400 ml-auto" aria-label="Remove from favorites"><StarIcon filled={true} /></button></li>)}
                </ul>
            ) : (
                <div className="flex-grow text-center p-8 bg-slate-800/50 rounded-lg border-2 border-dashed border-slate-700 flex flex-col items-center justify-center"><div className="text-amber-400 mb-3"><UserGroupIcon /></div><h4 className="font-semibold text-white">No Favorite DJs Yet</h4><p className="text-sm text-slate-400 mt-1">Star a DJ from the homepage to see them here!</p></div>
            )}
        </section>
    </div>
));

const RecentRequestsList = memo(({ songRequests }: { songRequests: SongRequestRecord[] }) => {
    const sortedSongRequests = useMemo(() => [...songRequests].sort((a, b) => new Date(b.requestedAt).getTime() - new Date(a.requestedAt).getTime()), [songRequests]);
            ...
            {sortedSongRequests.length > 0 ? (
                <div className="bg-slate-800/50 rounded-lg p-4">
                    <ul className="space-y-2">
                        {sortedSongRequests.map(req => (<li key={req.requestedAt} className="p-3 bg-slate-800 rounded-lg flex justify-between items-center"><div><p className="font-semibold text-white">{req.title}</p><p className="text-sm text-slate-400">{req.artist}</p></div><p className="text-xs text-slate-500 text-right">{formatDate(req.requestedAt)}<br/>at {formatTime(req.requestedAt)}</p></li>))}
                    </ul>
                </div>
            ) : (
