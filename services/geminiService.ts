...
const getApiKey = (): string => {
  ...
  if (!apiKey) {
    console.error("API_KEY environment variable not set. Please add it to your environment variables.");
    throw new Error("API_KEY environment variable not set.");
  }
  return apiKey;
};

// Create shared, memoized instances of the Gemini clients to optimize performance.
const ai = new GoogleGenAI({ apiKey: getApiKey() });
const ttsAi = new GoogleGenAI({ apiKey: getApiKey() }); // FIX: TTS model also requires API key in the browser.
